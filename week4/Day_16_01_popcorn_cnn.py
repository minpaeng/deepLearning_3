...
def get_train():
    df_train = pd.read_csv('../data/labeledTrainData.tsv',
...
def get_test():
    df = pd.read_csv('../data/testData.tsv', delimiter='\t', index_col=0)
    # print(df)
...
def make_submission(user_ids, predictions, filename):
    f = open(os.path.join('../data', filename), 'w', encoding='utf-8')
...
def load_ensemble(model_name1, model_name2, submission_name):
    ...
    p1 = p1.reshape(-1)       # (25000,)

    model2 = keras.models.load_model(model_name2)
    p2 = model1.predict(x_test_pad)
    p2 = p2.reshape(-1)  # (25000,)
...
load_model('../model/popcorn_02_0.32.h5', 'popcorn_rnn.csv')
# load_ensemble('../model/popcorn_03_0.34.h5', '../model/popcorn_cnn_04_0.30.h5', 'popcorn_cnn.csv')
